#!/bin/bash
mkdir /var/tmp/$HOSTNAME-PRE_Migration
cd /var/tmp/$HOSTNAME-PRE_Migration
nice -n 19 qkview -f /var/tmp/$HOSTNAME-PRE_Migration/$HOSTNAME.qkview
sleep 1m 
tmsh save sys ucs /var/tmp/$HOSTNAME-PRE_Migration/$HOSTNAME
sleep 15s
tmsh save sys config file /var/tmp/$HOSTNAME-PRE_Migration/$HOSTNAME no-passphrase 
sleep 15s
...
sleep 5s
echo '##xxxxxxxxxxxxxxxxxxxxxx GTM Pool Status xxxxxxxxxxxxxxxxxxxxxx##' >> $HOSTNAME.TMSH_Output
tmsh -q -c "cd /;show gtm pool recursive "| awk --posix  '/node-name/ {mem=$2} /addr/ {mbrAddr=$2} / port / {mbrPort=$2} /^ {12}status.availability-state/ {avStat=$2} /^ {12}status.enabled-state/ {enstate=$2} /^ {12}pool-name/ {pname=$2} /^ {8}}/ {print pname,mem,mbrAddr,mbrPort,avStat,enstate}' | sort -k 3,4 -k 1,1 >> $HOSTNAME.TMSH_Output
sleep 5s
...
sleep 5s
echo '##xxxxxxxxxxxxxxxxxxxxxx WIP Status xxxxxxxxxxxxxxxxxxxxxx##' >> $HOSTNAME.TMSH_Output
tmsh -q -c "cd /;show gtm wip recursive field-fmt" | awk --posix '/ name / {vsName=$2} /destination/ {dest=$2} /^ {4}status.availability-state/ {avStat=$2} /^ {4}status.enabled-state/ {print vsName,dest,avStat,$2}' | sed 's/:/ /' | sort -dk 2,2 -dk 3,3 >> $HOSTNAME.TMSH_Output
sleep 5s
echo '##xxxxxxxxxxxxxxxxxxxxxx IQuery Status xxxxxxxxxxxxxxxxxxxxxx##' >> $HOSTNAME.TMSH_Output
tmsh -q -c "cd /;show gtm iquery "  >> $HOSTNAME.TMSH_Output
sleep 5s
seq 5 | awk '{printf "\n"}' >> $HOSTNAME.TMSH_Output
